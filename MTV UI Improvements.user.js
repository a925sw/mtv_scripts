...
function initSeriesSearchBar(){
    $(searchbar_series).attr('id', 'searchbar_series').find("form").attr('name','series').removeAttr('action method').find("input").val("Series").attr({
        'id': 'series_search',
        'onfocus': 'if (this.value == "Series") { this.value = ""; }',
        'onblur': "if (this.value == '') { this.value = 'Series'; }",
        'placeholder': 'Series'});
    ...
    $(searchbar_series).submit(function(e){
        ...
        var id_match = series_id_list[input_str];
    ...
    $('#series_search').autocomplete({
        ...
        select: function(event, ui) {
            location.href = '/artist.php?id='+series_id_list[ui.item.value.toLowerCase()];
        }
    });
